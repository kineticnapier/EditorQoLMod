    ...
    internal static class TileHighlightOperations
    {
        ...
        internal static IEnumerable<KeyValuePair<string, string>> PropertyOptions(LevelEventType eventType)
        {
            ...
            try
            {
                ...
                IDictionary<string, object> data = sample.GetEventData();
                if (data != null)
                {
                    result.AddRange(data.Keys.OrderBy(x => x, StringComparer.OrdinalIgnoreCase)
            ...
            LevelEventType eventType, string property, bool includeDecorations)
        {
            ...
            try
            {
                ...
                IDictionary<string, object> data = sample.GetEventData();
                object defaultValue;
                if (data != null && data.TryGetValue(property, out defaultValue) &&
                    defaultValue != null)
                {
            ...
            TileHighlightComparison comparison, string expectedValue)
        {
            ...
            IDictionary<string, object> data = evnt.GetEventData();
            if (data == null) return false;
            object actual;
            if (!data.TryGetValue(property, out actual)) return false;
        ...
        private static string PropertyText(LevelEvent evnt, string property)
        {
            ...
            object value;
            return data.TryGetValue(property, out value) && value != null ? ValueText(value) : null;
        }
